...
// package.json
var sqlfluffVersion = "0.11.2";

// src/constant.ts
var SQLFLUFF_VERSION = sqlfluffVersion;
var SUPPORT_LANGUAGES = ["sql", "jinja-sql"];

// src/installer.ts
var exec = import_util.default.promisify(import_child_process.default.exec);
async function sqlfluffInstall(pythonCommand, context) {
  const pathVenv = import_path.default.join(context.storagePath, "sqlfluff", "venv");
  let pathVenvPython = import_path.default.join(context.storagePath, "sqlfluff", "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "python");
  }
  const statusItem = import_coc.window.createStatusBarItem(0, { progress: true });
  statusItem.text = `Install sqlfluff...`;
  statusItem.show();
  const installCmd = `${pythonCommand} -m venv ${pathVenv} && ${pathVenvPython} -m pip install -U pip sqlfluff==${SQLFLUFF_VERSION}`;
  import_rimraf.default.sync(pathVenv);
  try {
    import_coc.window.showMessage(`Install sqlfluff...`);
    await exec(installCmd);
    statusItem.hide();
    import_coc.window.showMessage(`sqlfluff: installed!`);
  } catch (error) {
    statusItem.hide();
    import_coc.window.showErrorMessage(`sqlfluff: install failed. | ${error}`);
    throw new Error();
  }
}
...
var LintEngine = class {
  constructor(cmdPath, outputChannel) {
    this.collection = import_coc2.languages.createDiagnosticCollection("sqlfluff");
  ...
  async lint(textDocument) {
    if (!SUPPORT_LANGUAGES.includes(textDocument.languageId))
      return;
    const self = this;
    const filePath = import_coc2.Uri.parse(textDocument.uri).fsPath;
    const args = [];
    const cwd = import_coc2.Uri.file(import_coc2.workspace.root).fsPath;
    const opts = { cwd, shell: true };
    const extensionConfig = import_coc2.workspace.getConfiguration("sqlfluff");
    args.push("lint");
    args.push("--format", "json");
    const ignoreParsing = extensionConfig.get("linter.ignoreParsing", true);
    if (ignoreParsing) {
...
`);
    ...
    return new Promise(function(resolve) {
      const cps = import_child_process2.default.spawn(self.cmdPath, args, opts);
        ...
        if (sqlfluffDiagnostics && sqlfluffDiagnostics.length > 0) {
          for (const d of sqlfluffDiagnostics) {
            for (const v of d.violations) {
              ...
              const startPosition = import_coc2.Position.create(lineNo, linePos);
              const endPosition = import_coc2.Position.create(lineNo, linePos);
              diagnostics.push({
                range: import_coc2.Range.create(startPosition, endPosition),
                severity: import_coc2.DiagnosticSeverity.Error,
...
async function doFormat(context, outputChannel, document, range) {
  ...
  const extensionConfig = import_coc3.workspace.getConfiguration("sqlfluff");
  let toolPath = extensionConfig.get("sqlfluff.commandPath", "");
  if (!toolPath) {
    if (import_fs.default.existsSync(import_path2.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe")) || import_fs.default.existsSync(import_path2.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff"))) {
      if (process.platform === "win32") {
        toolPath = import_path2.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe");
      } else {
        toolPath = import_path2.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff");
      }
    } else {
      throw "Unable to find the sqlfluff command.";
    }
  }
  const fileName = import_coc3.Uri.parse(document.uri).fsPath;
  const originalText = document.getText(range);
  const args = [];
  const opts = { cwd: import_path2.default.dirname(fileName) };
  args.push("fix");
  args.push("--force");
  args.push("-");
...
`);
  ...
  return new Promise((resolve) => {
    let newText = "";
    const cps = import_child_process3.default.spawn(toolPath, args, opts);
    cps.on("error", (err) => {
...
`);
        ...
        if (data.toString().match(/Unfixable violations detected/)) {
          ...
          import_coc3.window.showWarningMessage(`sqlfluff: ${JSON.stringify(data.toString().trim())}`);
        }
...
var SqlfluffFormattingEditProvider = class {
...
var format_default = SqlfluffFormattingEditProvider;

// src/action.ts
var import_coc4 = require("coc.nvim");
var SqlfluffCodeActionProvider = class {
  constructor(outputChannel) {
    this.source = "sqlfluff";
    this.diagnosticCollection = import_coc4.languages.createDiagnosticCollection(this.source);
    this.outputChannel = outputChannel;
  }
  async provideCodeActions(document, range, context) {
    const doc = import_coc4.workspace.getDocument(document.uri);
    const wholeRange = import_coc4.Range.create(0, 0, doc.lineCount, 0);
    let whole = false;
    ...
    if (this.lineRange(range) && context.diagnostics.length > 0) {
      ...
      if (line && line.length && !line.startsWith("--")) {
        const edit = import_coc4.TextEdit.replace(range, `${line} -- noqa${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
        ...
        const disableAllEdit = import_coc4.TextEdit.replace(range, `${line} -- noqa: disable=all${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
        ...
        const enableAllEdit = import_coc4.TextEdit.replace(range, `${line} -- noqa: enable=all${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
...
async function activate(context) {
  ...
  const extensionConfig = import_coc5.workspace.getConfiguration("sqlfluff");
  ...
  const outputChannel = import_coc5.window.createOutputChannel("sqlfluff");
  context.subscriptions.push(import_coc5.commands.registerCommand("sqlfluff.showOutput", () => {
    if (outputChannel) {
      outputChannel.show();
    }
  }));
  ...
  subscriptions.push(import_coc5.commands.registerCommand("sqlfluff.install", async () => {
  ...
  if (!sqlfluffPath) {
    ...
    } else if (import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe")) || import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff"))) {
      if (process.platform === "win32") {
        sqlfluffPath = import_path3.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe");
      } else {
        sqlfluffPath = import_path3.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff");
      }
    }
  }
  if (!sqlfluffPath) {
    if (pythonCommand) {
      ...
      import_coc5.window.showErrorMessage("python3/python command not found");
    }
    if (import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe")) || import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff"))) {
      if (process.platform === "win32") {
        sqlfluffPath = import_path3.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe");
      } else {
        sqlfluffPath = import_path3.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff");
  ...
  if (!sqlfluffPath) {
    setTimeout(() => {
      import_coc5.window.showErrorMessage('Exit, because "sqlfluff" does not exist.');
    }, 500);
    return;
  }
  ...
  if (onOpen) {
    import_coc5.workspace.documents.map(async (doc) => {
      await engine.lint(doc.textDocument);
    });
    import_coc5.workspace.onDidOpenTextDocument(async (e) => {
      await engine.lint(e);
    }, null, subscriptions);
  }
  const onChange = extensionConfig.get("lintOnChange");
  if (onChange) {
    import_coc5.workspace.onDidChangeTextDocument(async (_e) => {
  ...
  if (onSave) {
    import_coc5.workspace.onDidSaveTextDocument(async (e) => {
      await engine.lint(e);
    }, null, subscriptions);
  }
  ...
  function registerFormatter() {
    disposeHandlers();
    formatterHandler = import_coc5.languages.registerDocumentFormatProvider(languageSelector, editProvider, priority);
  }
  const isFormatEnable = extensionConfig.get("formatEnable");
  if (isFormatEnable) {
    registerFormatter();
  }
  context.subscriptions.push(import_coc5.commands.registerCommand("sqlfluff.fix", async () => {
    const doc = await import_coc5.workspace.document;
  ...
  context.subscriptions.push(import_coc5.languages.registerCodeActionProvider(languageSelector, actionProvider, "sqlfluff"));
}
async function installWrapper(pythonCommand, context) {
  const msg = 'Install/Upgrade "sqlfluff"?';
  context.workspaceState;
  let ret = 0;
  ret = await import_coc5.window.showQuickpick(["Yes", "Cancel"], msg);
  if (ret === 0) {
    try {
...
function getPythonPath(config, isRealpath) {
  ...
  try {
    ...
    if (isRealpath) {
      pythonPath = import_fs2.default.realpathSync(pythonPath);
    }
    return pythonPath;
  } catch (e) {
  }
